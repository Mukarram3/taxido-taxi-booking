...
class MessagesController extends Controller
{
    ...
    public function index( $id = null)
    {
        ...
        $messenger_color = Auth::guard('user')->user()->messenger_color;
        return view('Chatify::pages.app', [
            'id' => $id ?? 0,
            'messengerColor' => $messenger_color ? $messenger_color : Chatify::getFallbackColor(),
            'dark_mode' => Auth::guard('user')->user()->dark_mode < 1 ? 'light' : 'dark',
        ]);
    }
    ...
    public function send(Request $request)
    {
        ...
        if (!$error->status) {
            $message = Chatify::newMessage([
                'from_id' => Auth::guard('user')->user()->id,
                'to_id' => $request['id'],
            ...
            if (Auth::guard('user')->user()->id != $request['id']) {
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', Auth::guard('user')->user()->id);
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('id','!=',Auth::guard('user')->user()->id)
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? User::where('id', Auth::guard('user')->user()->id)->update(['dark_mode' => 1])  // Make Dark
                : User::where('id', Auth::guard('user')->user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed_images)) {
                    ...
                    if (Auth::guard('user')->user()->avatar != config('chatify.user_avatar.default')) {
                    ...
                    $update = User::where('id', Auth::guard('user')->user()->id)->update(['avatar' => $avatar]);
