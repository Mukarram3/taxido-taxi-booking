...
@section('content')
    ...
    @php
        $driverriderequest = \App\Models\Driverriderequest::find($driverriderequest_id);
    @endphp
    ...
    <header id="header" class="main-header inner-page-header">
        <div class="custom-container">
            <div class="header-panel">
                ...
                <a href="#alert" data-bs-toggle="offcanvas" data-id="{{ $driverriderequest->id }}" class="cancel_offerBtn">
...
{{--        <script src="{{asset('assets/js/custom-map.js')}}"></script>--}}
        ...
        <script>
            const userId = {{ Auth::guard('driver')->id() }};
            const driverriderequest_id = {{ $driverriderequest_id }};
            ...
            function initMap() {
                ...
                function getUserFareRequest() {
                    $.ajax({
                        url: `/driver/get-user-fare-request?driverriderequest_id=${driverriderequest_id}`,
                        method: 'GET',
                        success: function(response) {
                            if (response.ridebooked) {
                                toastr.success("Ride booked");
                                window.location.href = '{{ url('/driver/home') }}';
                                return;
                            }
                            ...
                            <div class="driver-box">
                                <div class="profile-head">
                                    <div class="flex-align-center gap-2">
                                        <img class="img-fluid profile-img" src="${item.user?.profile ? '/storage/' + item.user.profile : '/assets/images/profile/p8.png'}" alt="profile">
                                    </div>
                                    <div class="flex-align-center gap-2 d-flex flex-row">
                                        <h4 class="fw-medium success-color d-block">${item.driverriderequest?.price_currency}${item.requested_fare}/Kilo</h4>
                                            <br>
                                        <h4 class="fw-medium success-color d-block">Requested Kilos ${item.requested_kilos} Kilo</h4>
                                    </div>
                                </div>
                                <div class="flex-spacing mt-2">
                                    <h5 class="fw-normal title-color">${item.user?.name ?? 'Unknown User'}</h5>
                                ...
                                <div class="grid-btn mt-2">
                                    <a href="#" id="reject_request" data-user_id="${item.user_id}" data-userfarerequest_id="${item.id}" class="btn gray-btn w-100 m-0">Skip</a>
                                    <a href="/driver/accept-ride-details?userfarerequest_id=${item.id}&user_id=${item.user_id}" class="btn theme-btn w-100 m-0">Accept</a>
                                </div>
                    ...
                    `;
                                ...
                                $('#driverFareList').html('<li><p><div class="alert alert-success" role="alert">Users search in progress</div></p></li>');
                            }
                        },
                        error: function(xhr) {
