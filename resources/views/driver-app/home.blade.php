                        ...
                        <div class="text-center my-3">
                            ...
                            @php
                                if (\Illuminate\Support\Facades\Auth::guard('driver')->check()){
                                    $driver_request = \App\Models\Farerequest::where('driver_id',\Illuminate\Support\Facades\Auth::guard('driver')->user()->id)->where('riderequest_id', $userriderequest->id)->orderby('id','desc')->first();
                                    if ($driver_request && $driver_request->status == 'waiting' && $userriderequest->id == $driver_request->riderequest_id){
