    ...
    <div class="filters-section">
        ...
        <div class="filter-tabs">
            ...
            <div class="filter-tab" onclick="filterProposals('finished', event)">
                ...
                <span data-lang="en">Finished</span>
                <span class="filter-count">{{ count($completed_rides) }}</span>
            </div>
            <div class="filter-tab" onclick="filterProposals('cancelled', event)">
                <span data-lang="fr" class="active">Refusées</span>
    ...
    <div class="proposals-list" id="proposalsList">
        ...
        @foreach($active_rides as $offer)
            @php
                ...
                $transportName = ucfirst(strtolower($offer->userriderequest->vehicle_type_needed ?? ''));
                $icon = $icons[$transportName] ?? '❓'; // default fallback
            @endphp
            <div class="proposal-card" data-status="in-progress">
                ...
                {{ $icon }} {{ $offer->pickup_city }} → {{ $offer->destination_city }}
            ...
            <div class="proposal-card" data-status="accepted">
                ...
                <div class="proposal-header">
                    ...
                    <div class="proposal-price">
                        ...
                        <div class="price-amount">{{ $ride->fare }}€</div>
                        <div class="price-original">
                            <span data-lang="fr" class="active">Initial: {{ $ride->fare }}€</span>
                            <span data-lang="en">Initial: €{{ $ride->fare }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="proposal-card" data-status="finished">
                <div class="proposal-header">
                    ...
                    <div class="proposal-price">
                        ...
                        <div class="price-amount">€{{ $completed_ride->fare }}</div>
                    </div>
                </div>
                @php
                ...
                {{ $completed_ride->pickup_city }} -> {{ $completed_ride->destination_city }}

                <div class="custom-section">
                    <div>
                        {{ $icon }} <span class="custom-label">Transport:</span> {{ $completed_ride->transport_title }}
                    </div>
                    ...
                    <div>⏰ <span
                            class="custom-label">Follow-up:</span> {{ $completed_ride->date_and_time_of_followup }}
                    </div>
                </div>

                <div class="proposal-actions">
                    <a class="btn btn-primary" href="{{ url('user/ride-details?ride_id='.$completed_ride->id) }}">
        ...
        @foreach($cancelled_rides as $cancelled_ride)
            ...
            <div class="proposal-card" data-status="cancelled">
                ...
                {{ $cancelled_ride->pickup_city }} -> {{ $cancelled_ride->destination_city }}

                <div class="custom-section">
                    <div>
                        {{ $icon }} <span class="custom-label">Transport:</span> {{ $cancelled_ride->transport_title }}
                    </div>
                    ...
                    <div>⏰ <span
                            class="custom-label">Follow-up:</span> {{ $cancelled_ride->date_and_time_of_followup }}
                    </div>
                </div>

                <div class="proposal-actions">
                    <a class="btn btn-primary" href="{{ url('user/ride-details?ride_id='.$cancelled_ride->id) }}">
                        <img src="{{ asset('assets/images/view-detail-image.png') }}"
                             title="View package details" width="30px" alt="loading">
                    </a>
