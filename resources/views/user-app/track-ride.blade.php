        ...
        <div class="transport-details-card">
            ...
            <div class="details-grid">
                ...
                <div class="detail-item">
                    ...
                    <span class="detail-value">{{ $icon }}️ Air France AF022</span>
                </div>
                @php
    ...
    <span class="info-label">
        ...
        <div class="contact-card">
            <div class="contact-header">
                <img src="{{ $track_ride->driver->profile ? asset('storage/'.$track_ride->driver->profile) : asset('assets/images/profile/p8.png') }}" alt="Thomas"
                     class="contact-avatar">
                <div class="contact-info">
                    <div class="contact-name">{{ $track_ride->driver->firstName . ' ' . $track_ride->driver->lasttName }}</div>
                    <div class="contact-role">
                        <span class="lang-content fr active">Carrier</span>
...
<script>
    ...
    function initMap() {
        ...
        if (rideMessage === 'On the way to pickup' ||
            rideMessage === 'User Requested to Return the Package' ||
            rideMessage === 'Carrier Cancelled Ride' ||
            rideMessage === "Package return in progress") {
    ...
    function calculateRoute() {
        ...
        directionsService.route({
            ...
            if (status === "OK") {
                ...
                $('#edr-info').html((totalMeters / 1000).toFixed(2) + " km");

                let remainingSeconds = totalSeconds;
                let hours = Math.floor(remainingSeconds / 3600);
                let minutes = Math.ceil((remainingSeconds % 3600) / 60);

                $('#etr-info').html(`~${hours}h ${minutes}min`);
            }
        });
    }
    ...
    function startTracking() {
        ...
        navigator.geolocation.watchPosition(
            position => {
                ...
                if (!pickupLatLng) pickupLatLng = currentLatLng;

                // Distance traveled update
                if (previousPosition) {
                    const prevLatLng = new google.maps.LatLng(previousPosition.lat, previousPosition.lng);
                    totalDistanceTraveled += google.maps.geometry.spherical.computeDistanceBetween(prevLatLng, currentLatLng);
                }

                // Distance from pickup
                const distanceFromPickupMeters =
                    google.maps.geometry.spherical.computeDistanceBetween(pickupLatLng, currentLatLng);

                // Distance remaining based on steps
                let distanceRemaining = 0;
                distanceRemaining = allSteps.reduce((sum, step) => sum + step.distance.value, 0);

                // ETA estimation (rough)
                const avgSpeed = 40 * 1000; // assume 40 km/h → 40,000 m/h
                const etaSeconds = distanceRemaining / (avgSpeed / 3600);

                let remH = Math.floor(etaSeconds / 3600);
                let remM = Math.ceil((etaSeconds % 3600) / 60);

                // Update DOM
                document.getElementById('distance-traveled').innerText =
                    (distanceFromPickupMeters / 1000).toFixed(2) + " km";

                document.getElementById('current-altitude').innerText =
                    altitude !== null ? `${altitude.toFixed(0)} m` : 'N/A';

                document.getElementById('edr-info').innerText =
                    (distanceRemaining / 1000).toFixed(2) + " km";

                document.getElementById('etr-info').innerText =
                    `~${remH}h ${remM}min`;
